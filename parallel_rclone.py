...
def get_srcdst_dirs(src_dir):
    ...
    if 'zenas_pool03' in dst_dirpath:
        dst_dirpath = dst_dirpath.replace('zenas_pool03/','')
...
def run_rclone(src_dirpath, dst_remote, dst_dirpath, logfile, dry_run=True):
    """Run rclone.
...
def start_backup(srcbase_dirpath,dst_remote,dstbase_dirpath):
    ...
    Args:
		...
		dst_remote (str)|: Rclone's defined remote
    """
    ...
    while counter > 0:
        ...
        if src_dirpath == 'no-dir':
            ...
            counter += 1
            dir_count += 1
            pool.apply_async(backup_worker, (srcbase_dirpath, src_dirpath, dst_remote,
											 dstbase_dirpath, src_dirpaths))
...
def backup_worker(srcbase_dirpath, src_dirpath, dst_remote, dstbase_dirpath, src_dirpaths):
    ...
    Args:
		...
		dst_remote (str): Rclone's configured remmote
    """
...
if __name__ == "__main__":
    ...
    # Start backup
    if sizetobackup > 0:
        start_backup(src_dirpath, DST_REMOTE, dst_dirpath)
